...
class BarCharts(BaseEstimator, TransformerMixin):
    ...
    def plot(self, X):
        ...
        temp_X = X.sort_values(by = ['Information_Value','Variable_Name'], ascending = False).copy(deep=True)
    ...
    def _plot(self, X):
        ...
        grouped = X.sort_values(by = ['Information_Value','Variable_Name'], ascending = False).groupby(['Variable_Name'], sort = False)
